...
export function generateOptions(marketType, homeTeam, awayTeam) {
  ...
  if (mt === 'btts') {
    return [{ label: 'BTTS Yes', value: 'BTTS Yes' }, { label: 'BTTS No', value: 'BTTS No' }];
  }
...
// Copy to Admin too
export function getOptionsLayout(marketType) {
  const mt = normalizeType(marketType);
  if (mt.includes('corner') || mt.includes('goal') || mt.includes('card') || mt.includes('shot')) {
    return 'grid';   // OV/UN grid
  }
  if (mt.includes('result') || mt === 'scores') return 'three'; // 3-column
  return 'two';      // YES/NO
}
