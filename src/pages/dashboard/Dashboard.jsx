...
export default function Dashboard() {
  ...
  const stats = [
    { icon: FiList,       label: 'Active Tickets', value: activeCount,            color: 'text-[#1A4D8F]',  bg: 'bg-blue-50',   border: 'border-l-[#1A4D8F]' },
    { icon: FiDollarSign, label: 'Total Winnings',  value: `$${totalWon.toFixed(2)}`, color: 'text-green-600', bg: 'bg-green-50', border: 'border-l-green-400' },
    { icon: FiTrendingUp, label: 'Wallet Balance',  value: `$${(user?.balance ?? 0).toFixed(2)}`, color: 'text-purple-600', bg: 'bg-purple-50', border: 'border-l-purple-400' },
    { icon: FiClock,      label: 'Total Tickets',   value: tickets.length,         color: 'text-orange-600', bg: 'bg-orange-50', border: 'border-l-orange-400' },
  ];
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
        ...
        {loading ? (
          ...
          <div className="text-center py-10">
            <FiList className="w-8 h-8 text-gray-200 mx-auto mb-2" />
            <p className="text-gray-400 text-sm">No tickets yet</p>
            <Link to="/lobby" className="text-[#1A4D8F] text-sm font-medium hover:underline mt-1 inline-block">Browse Lobby</Link>
          </div>
        ) : (
